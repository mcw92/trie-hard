...
class PrefixMatcher:
    ...
    def __init__(
            self,
            list_of_words: Union[List[str], np.ndarray[str]],
            prefix: str
    ) -> None:
...
if __name__ == "__main__":
    ...
    list_of_words = np.loadtxt(path, dtype=str, comments="#")  # Load list of words from txt file.
    rng.shuffle(list_of_words)  # Shuffle list.

    cpu_count = mp.cpu_count()  # Get number of CPUs available.
    pool = mp.Pool(processes=cpu_count)  # Create a process pool with the number of CPU cores available.

    # Split the string_list into chunks for parallel processing
    chunk_size = len(list_of_words) // cpu_count
    chunks = [list_of_words[i:i + chunk_size] for i in range(0, list_of_words.shape[0], chunk_size)]

    # Use the pool.map() function to apply the function to each chunk in parallel.
    results = pool.starmap(find_matches_list_comprehension, [(prefix, chunk) for chunk in chunks])
    print(results)
